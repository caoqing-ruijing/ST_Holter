...
class SwinTransformerBlock(nn.Module):
    ...
    def __init__(self, dim, input_resolution, num_heads, window_size=7, shift_size=0,
        ...
        # print('input_resolution',input_resolution,'window_size',window_size,'shift_size',shift_size)
...
class Model(nn.Module):
    def __init__(self, in_c=1,out_c=1,
        ...
        self.encode_patch_embed = PatchEmbed(
            img_size=img_size, patch_size=patch_size, in_chans=in_c, embed_dim=embed_dim,
            norm_layer=nn.LayerNorm)

        patches_resolution = self.encode_patch_embed.patches_resolution
        self.patches_resolution = patches_resolution
        ...
        self.decode_layers_up = nn.ModuleList()
        self.decode_concat_back_dim = nn.ModuleList()
        for i_layer in range(self.num_layers):
            ...
            self.decode_layers_up.append(layer_up)
            self.decode_concat_back_dim.append(concat_linear)

        self.decode_final_swin_layer = BasicLayer(
                        dim=int(embed_dim * 2),
                        input_resolution=(1,patches_resolution[0] // (2 ** (self.num_layers-1-i_layer))),
                        depth=depths[(self.num_layers-1-i_layer)],
                        num_heads=num_heads[(self.num_layers-1-i_layer)],
        ...
        # print('int(patch_size*0.5)',int(patch_size*0.5))
        # print('embed_dim // int(patch_size*0.5)',embed_dim // int(patch_size*0.5))
        if patch_size > 1:
    ...
    def forward(self, input):
        ...
        x_stem = self.encode_patch_embed(input)
        # print('encode_patch_embed',x_stem.shape)
        ...
        for i,layer in enumerate(self.encode_layers):
            ...
            x = layer(x)
            # print('encode_layers',i,x.shape)

        x = self.norm(x)  # B L C
        # print('x decode last layer',x.shape)
        for inx, layer_up in enumerate(self.decode_layers_up):
            # print('inx',inx,x.shape)
            if inx == 0:
                x = layer_up(x)
            else:
                # x = torch.cat([x,x_downsample[3-inx]],-1)
                x = torch.cat([x,x_downsample[len(x_downsample)-1-inx]],-1)
                x = self.decode_concat_back_dim[inx](x)
                x = layer_up(x)
            # print('decode_layers_up--->',x.shape)

        # print('x_stem',x_stem.shape)
        x = torch.cat([x,x_stem],-1)
        # print('x cat',x.shape)
        # x = self.final_layer_up(x)
        x = self.decode_final_swin_layer(x)
        # print('x decode_final_swin_layer',x.shape) ##([2, 3584, 24])
        if x.shape[1] != input.shape[1]:
            x = self.layer_up(x)
            # print('x final_layer_up',x.shape) ##([2, 3584, 24])

        x = self.norm_up(x)  # B L C
        x = x.transpose(1, 2)  #B,C,H,W 

        # print('x before reg',x.shape)
        fc_out = self.reg(x)
